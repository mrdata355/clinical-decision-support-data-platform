...
const { json, requestId, deploymentMeta, readJson } = require('../server/http');
...
function guardSql(sql) {
  ...
  const refs = [...statement.matchAll(ALLOWED_SCHEMA)];
  if (!refs.length) throw new Error('GOVERNED_SCHEMA_REQUIRED');
  if (/\bfrom\s+(RAW|STAGING)\./i.test(statement) || /\bjoin\s+(RAW|STAGING)\./i.test(statement)) {
    throw new Error('RAW_STAGING_ACCESS_DENIED');
  }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method !== 'POST') return json(res, 405, { error: 'METHOD_NOT_ALLOWED', requestId: rid });
  try {
    ...
    return json(res, 200, {
      ...
      execution: {
        ...
        note: 'Execution is intentionally disabled in the public portfolio deployment until approved warehouse credentials are configured.',
      },
      serverComputeMs: Number((performance.now() - started).toFixed(2)),
    });
  } catch (error) {
    return json(res, 400, { error: error.message || 'SQL_BOT_ERROR', requestId: rid });
  }
};
